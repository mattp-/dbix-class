...
#
# If the RDBMS for the composing class defines NULLs as unique values in unique
# constraints, the class should define:
#
#   sub _nullable_unique_constraints_are_unique { 1 }
#
# This is true in, for example, Sybase ASE and MSSQL.
#
# Returns the smallest possible identifying condition, giving preference to the
# primary key.

sub _nullable_unique_constraints_are_unique { 0 }
...
sub _ident_cond_for_cols {
  ...
  my $nulls_are_unique = $self->_nullable_unique_constraints_are_unique;

  my $cols;

  USET: foreach my $set (delete $unique{primary},
                          sort { @$a <=> @$b } (values %unique)) {
    next unless $set && @$set;

    foreach my $col (@$set) {
      next USET if not exists $row->{$col};

      next USET if $colinfos->{$col}{is_nullable} && (not defined $row->{$col})
                  && (not $nulls_are_unique);
    }
